...
export class Middleware<S extends State> {
    preActionsMap: Map<string, Array<(Store: Store<S>, action: Action) => Store<S>>> = new Map();
    postActionsMap: Map<string, Array<(Store: Store<S>, action: Action, prevState: S) => Store<S>>> = new Map();

    private executePre(store: Store<S>, action: Action, actionType?: string) {
    ...
    private executePost(store: Store<S>, action: Action, prevState: S, actionType?: string) {
